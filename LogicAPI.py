...
class Term(object):
    ...
    def query(self):
        key = (self.functor, len(self.args))
        if key not in kb:
            raise Exception('Undefined procedure: ' + str(self.functor) + '/' + str(len(self.args)))
        rules = kb[key]
        for left, rights in rules:
            ...
            if rights:
                ...
                rights_new += [self.unique(int_names, right) for right in rights]
                rights = rights_new
            env = {}
...
class Terms(list):
    ...
    def __invert__(self):
        ...
        return self
    def __repr__(self):
        res = ' & '.join([repr(term) if isinstance(term, Term) else '(' + repr(term) + ')' for term in self])
        if self.inverted:
            return '~(' + res + ')'
        else:
            return res
    ...
    def eval(self):
        for i in xrange(len(self.args)):
            if isinstance(self.args[i], Var):
                raise Exception('There are variables in the function: ' + str(self))
...
class EmptyList(Const):
    ...
    def __add__(self, other):
        if isinstance(other, List):
            ...
            raise Exception('[] and ' + repr(other) + ' cannot be concatenated.')
    def __repr__(self):
        return '[]'
...
class List(object):
    ...
    def toPythonList(self):
        if self.rest:
            raise Exception(repr(self.rest) + ' in ' + repr(self) + ' is unknown.')
        acc = []
        temp = self.first
        while temp:
    ...
    def __add__(self, other):
        ...
        if not isinstance(other, List):
            raise Exception(repr(self) + ' and ' + repr(other) + ' cannot be concatenated.')
        if self.rest:
            raise Exception(repr(self.rest) + ' in ' + repr(self) + ' is unknown.')
